...
class AccountBookPlugin(Star):
    ...
    async def add_record(self, event: AstrMessageEvent, *args: str) -> MessageEventResult:
        ...
        try:
            amount = float(args[1])
            if amount <= 0:
                return MessageEventResult(message="❌ 金额必须大于0")
        except ValueError:
            return MessageEventResult(message="❌ 金额必须是正数")
        ...
        try:
            ...
            return MessageEventResult(message="❌ 日期格式应为YYYY-MM-DD")
        ...
        return MessageEventResult(message=f"✅ 记录成功: {date} {category} {amount}元")
    ...
    async def query_records(self, event: AstrMessageEvent, date: str = None) -> MessageEventResult:
        ...
        try:
            ...
            return MessageEventResult(message="❌ 日期格式错误\n" + self._show_help("query").message)
        ...
        if not matched:
            return MessageEventResult(message=f"📭 {title}没有记录")
    ...
    async def show_stats(self, event: AstrMessageEvent) -> MessageEventResult:
        ...
        if not self._records:
            return MessageEventResult(message="📭 暂无任何记录")
...
"""
        ...
        if cmd_type in help_texts:
            return MessageEventResult(message=help_texts[cmd_type].strip())
        return MessageEventResult(message=help_texts["default"].strip())

    @filter.command("导出")
    async def export_data(self, event: AstrMessageEvent) -> MessageEventResult:
        """导出所有记录(开发中)"""
        return MessageEventResult(message="⏳ 导出功能开发中，敬请期待")
